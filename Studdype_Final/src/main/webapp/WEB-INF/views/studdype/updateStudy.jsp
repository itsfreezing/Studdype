...
function asdf(){
   var selectgu = $("#locationgu option:selected").val();
   
   if($("#locationgu option:selected").val()==0){
      alert("구를 선택해주세여!");
      return false;
   }else if($("#studyinfot").val().trim() == ""){
      alert("스터디 소개를 작성해주세요!");
      return false;
   ...
   $(function() {
      // 시 미선택 시 구/군은 숨김 
      $("#locationgu option").hide();

      // 시 option 선택 시 해당 시에 대한 구/군 리스트 보여주기 (전에 보여준 구/군은 다시 숨김)
      $("#locationsi").change(function() {
         var selectSi = $("#locationsi option:selected").val();

         $("#locationgu option").hide();
         $(".normal_gu").prop("selected",true);
         $("." + selectSi).show();
      });
...
<body>
   ...
   <div id="main" >
   <form method="post" enctype="multipart/form-data" action="studyupdate.do" autocomplete="off" onsubmit="return asdf();">
      ...
      <div id="studyimage" class="image-container" >
         <img src="${fileName }" style="height:100%; width:100%;" onError="this.src='./resources/img/no-image1.png'" value="${fileName }">
         <input type="hidden" name="photo" value="${img.getPhoto() }">
      ...
      <select id="locationgu" name="gu_no">
         <option  class="normal_gu" value="0">(구/선택)</option>
         <c:forEach var="gudto" items="${gudto }">
         
            <option value="${gudto.gu_no }" class="${gudto.si_no }" <c:if test="${gudto.gu_no == study.gu_no }">selected </c:if>>${gudto.gu_name}</option>
   ...
   <div id="main2" style="display:none;">
      ...
      <p id="p3" style="color:#808080; position:absolute; left:1540px; top:555px; cursor:pointer;" onclick="main3();">대표 도서 지정</p>
      <p style="position:absolute; top:63%; left:28%; color:black; font-size:20px; font-weight:bold;">스터디 전체 멤버</p>
      
      <div id="scr" style=" width:1117px; position:absolute; top:70%; left:28%; height:600px; overflow-x:hidden; border-radius:4px; ">
      <table id="member"
   ...
   <div id="main3" >
      ...
      <p style="position:absolute; top:63%; left:28%; color:black; font-size:20px; font-weight:bold;">전체 도서 </p>
   
   <div id="scr"style=" width:1117px; position:absolute; top:70%; left:28%; height:600px; border-radius:4px; ">
   <table id="book1"
         class="table table-bordered" style="width:1117px; text-align:center;">
