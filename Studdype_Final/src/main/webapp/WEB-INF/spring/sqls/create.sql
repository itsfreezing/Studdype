...
CREATE SEQUENCE BOOKREPLYSEQ NOCACHE;  --자유게시판 댓글
CREATE SEQUENCE BOOKGROUPSEQ NOCACHE;  --자유게시판 댓글 그룹
...
CREATE SEQUENCE DATAFILESEQ NOCACHE;  --자료실 게시판 게시글 사진
CREATE SEQUENCE DATAGROUPSEQ NOCACHE; -- 자료실 댓글 그룹순서 시퀀스
...
SELECT * FROM PHOTO_FILE;
UPDATE PHOTO_FILE SET PHOTO_ISMAIN = 'Y' WHERE F_NO = (SELECT MIN(F_NO) FROM PHOTO_FILE WHERE B_NO = 4);
SELECT MIN(F_NO) FROM PHOTO_FILE WHERE B_NO =4;
COMMIT;
